...
export const Ingredient = ({ ingredient, showPrice, ...props }) => {
  ...
  // Safeguard against missing ingredients in INGREDIENTS
  const ingredientData = INGREDIENTS[ingredient.name] || {};

  return (
    <animated.group {...props} scale={scale} position-y={positionY}>
      {showPrice && ingredientData.total && (
        <Suspense>
          <group
            onClick={(e) => {
              e.stopPropagation();
          ...
          >
            ...
            <Text3D
              ...
              position-x={0.42}
            >
              ${ingredientData.total?.toFixed(2) || "0.00"}
            </Text3D>
            <Text3D
              font={fontPath}
              scale={0.1}
